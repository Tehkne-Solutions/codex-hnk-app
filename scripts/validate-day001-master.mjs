...
const required = [
  ...
  'apps/web/app/day-001/web-live.module.css',
...
const webLiveCss = readFileSync('apps/web/app/day-001/web-live.module.css', 'utf8');
...
const invariants = [
  ...
  ['Web route uses authenticated runtime provider', webPage.includes('<WebDay001RuntimeProvider>') && webPage.includes('<WebAtriumBoundary>')],
  ['Web proof labels local state truthfully', webMaster.includes('WEB PROOF · LOCAL STATE')],
  ['Web live labels Vault boundary truthfully', webMaster.includes('WEB LIVE · VAULT FINAL BLOQUEADO')],
  ...
  ['Web runtime uses explicit shared auth client', webRuntime.includes('createHnkSupabaseClient') && webRuntime.includes('parseAuthCallbackUrl')],
  ...
  ['Web auth storage is browser-local but Vault is not', webRuntime.includes('window.localStorage') && !webMaster.includes('localStorage')],
  ['Web validates canonical raw markdown before live crossing', webMaster.includes('raw_markdown') && webMaster.includes('crossingAwaitingCanon')],
  ['Web starts a real practice session only after canon validation', webMaster.includes('startPracticeSession') && webMaster.includes('runtime.userId && canon && !practice')],
  ['Web declares volatile private text truthfully', webMaster.includes('MEMÓRIA VOLÁTIL DO BROWSER · NÃO PERSISTIDA · NÃO ENVIADA')],
  ['Web blocks final Vault seal by design', webMaster.includes('SELO WEB BLOQUEADO COM SEGURANÇA') && webMaster.includes('keystore/recovery E2EE')],
  ['Web live path cannot write encrypted Vault yet', !webMaster.includes('saveEncryptedVaultEntry') && !webRuntime.includes('saveEncryptedVaultEntry')],
  ['Web live path cannot write practice completion yet', !webMaster.includes('savePracticeRecord') && !webRuntime.includes('savePracticeRecord')],
  ['Web live path cannot award completion XP yet', !webMaster.includes('completeCodexDay') && !webRuntime.includes('completeCodexDay')],
  ...
  ['Web auth gate uses semantic tokens', webRuntimeCss.includes('var(--hnk-kether-void)') && webRuntimeCss.includes('var(--hnk-font-sacred-display)')],
  ['Web live boundary uses semantic tokens', webLiveCss.includes('var(--hnk-kether-gold-bright)') && webLiveCss.includes('var(--hnk-font-system)')],
